    ...
    if (mailingsData) {
      const mailingsWithRecipients = await Promise.all(
        mailingsData.map(async (mailing) => {
          // Подгружаем получателей без предположений о наименовании FK в JS — используем alias "email" (т.к. FK в БД называется email_id)
          ...
          // Нормализуем: PostgREST положил связанную почту под ключ "email" (т.к. FK email_id),
          // а в UI мы ожидаем recipient.sender_email
          const normalized = (recipients || []).map((r: any) => ({
            ...r,
            // r.email может быть объектом email или undefined
            sender_email: r.email || null,
            // явно оставить поле email_id (как в БД)
